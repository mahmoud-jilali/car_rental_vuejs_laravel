...
const openModal = (brand = null, action = 'create') => {
    if (action === 'update') {
        isUpdate.value = true;
...
<template>
    ...
    <div class="flex mx-auto justify-between my-14 w-11/12">
        <h3 class="uppercase text-4xl font-bold text-blue-500 md:text-4xl ml-60">Brands</h3>
    ...
    <template v-if="brandsStore.loading">
        <div role="status">
    ...
    <template v-else-if="brandsStore.brand">
        <table class="w-3/4 ml-72 text-sm text-left rtl:text-right text-gray-900 cursor-pointer">
            ...
            <tbody>
                <tr v-for="(brand, row) in brandsStore.brand" class="bg-white border-b border-x"
                @mouseover="showActions(row)" @mouseleave="hideActions">
                    ...
                    <td class="px-6 py-4 flex">
                        ...
                        <div v-if="actions === row" class="absolute rigth-auto border rounded-lg w-auto mt-8 bg-gray-50">
                            <button @click="openModal(brand, 'update')" class="p-2 flex hover:bg-gray-200 cursor-pointer w-full">
                                <FontAwesomeIcon icon="pen-to-square" class="mr-1 py-1 cursor-pointer text-green-500" />Edit
                            </button>
                            <button @click="openModal(brand, 'delete')" class="p-2 flex hover:bg-gray-200 cursor-pointer w-full">
    ...
    <Update :show="showModal && isUpdate" :brand="selectedBrand" @close="closeModal" @updated="closeModal" />
    <Delete :show="showModal && isDelete" :brand="selectedBrand" @close="closeModal" @deleted="closeModal" />
    <RouterView />
</template>