...
const openModal = (insurance = null, action = 'create') => {
    if (action === 'update') {
        isUpdate.value = true;
...
<template>
                ...
                @mouseover="showActions(row)" @mouseleave="hideActions">
                    ...
                    <td class="px-6 py-4 flex">
                        ...
                        <div v-if="actions === row" class="absolute rigth-auto border rounded-lg w-auto mt-8 bg-gray-50">
                            <button @click="openModal(insurance, 'update')" class="p-2 flex hover:bg-gray-200 cursor-pointer w-full">
                                <FontAwesomeIcon icon="pen-to-square" class="mr-1 py-1 cursor-pointer text-green-500" />Edit
                            </button>
                            <button @click="openModal(insurance, 'delete')" class="p-2 flex hover:bg-gray-200 cursor-pointer w-full">
    ...
    <Update :show="showModal && isUpdate" :insurance="selectedInsurance" @close="closeModal" @updated="closeModal" />
    <Delete :show="showModal && isDelete" :insurance="selectedInsurance" @close="closeModal" @deleted="closeModal" />
