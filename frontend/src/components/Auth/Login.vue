...
<template>
    ...
    <div class="flex min-h-full my-24 bg-blue-50 mx-auto w-2/3 flex-1 flex-col justify-center px-6 py-12 lg:px-8 rounded-lg">
        <div class="sm:mx-auto sm:w-full sm:max-w-sm">
            ...
            <h2 class="my-5 text-center text-2xl font-bold leading-9 tracking-tight text-gray-900">Sign in to your account
            </h2>
        </div>
        <div class="my-5 sm:mx-auto sm:w-full sm:max-w-sm">
            <form class="space-y-6" @submit.prevent="authStore.login(form)"> 
                ...
                <div class="relative z-0 w-full my-2 group">
                    <div class="flex">
                        <input name="password" id="password" v-model = "form.password" :type="showPassword ? 'text' : 'password'"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-blue-500 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " />
                            <FontAwesomeIcon :icon="showPassword ? 'fa-regular fa-eye-slash' : 'fa-regular fa-eye'" @click="showHidePassword()"
                            class="py-3 border-0 border-b-2 border-blue-500 cursor-pointer" />
                        <label for="password"
                        class="peer-focus:font-medium absolute text-sm text-gray-900 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-5">
                        Password</label>
                    </div>
                ...
                <div class="text-sm">
                    <RouterLink to="/forgot_password" class="font-semibold text-blue-700 hover:text-blue-800">Forgot password?</RouterLink>
                </div>
                <div class="flex items-center mx-2">
                    <button type="submit" :disabled="authStore.loading"
                        class="text-white bg-blue-500 hover:bg-white hover:text-blue-500 hover:border border-blue-500 
                        font-medium rounded-lg text-sm mx-4 px-4 py-2 inline-flex text-center cursor-pointer">
                        <template v-if="!authStore.loading">Log in</template>
                        <template v-else>
                            <div role="status">
                    ...
                    <p class="my-3 text-center text-sm text-gray-500">You don't have account yet?
                        <RouterLink to="/register" class="font-semibold leading-6 text-blue-700 hover:text-blue-800">Create yours</RouterLink>
                    </p>
                </div>
            </form>
